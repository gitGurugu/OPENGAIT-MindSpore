...
def cross_view_gallery_evaluation(feature, label, seq_type, view, dataset, metric):
    ...
    for (type_, probe_seq) in probe_seq_dict[dataset].items():
        ...
        for (v1, probe_view) in enumerate(view_list):
            ...
            if hasattr(dist, 'asnumpy'):
                dist = dist.asnumpy()
            eval_results = compute_ACC_mAP(
                dist, probe_y, gallery_y, view[pseq_mask], view[gseq_mask])
        ...
        for (v1, probe_view) in enumerate(view_list):
            ...
            for (v2, gallery_view) in enumerate(view_list):
                ...
                if hasattr(dist, 'asnumpy'):
                    dist = dist.asnumpy()
                # Use numpy for topk
                idx = np.argsort(dist, axis=1)[:, :num_rank]
...
def HID_submission(data, dataset, rerank=True, metric='euc'):
    ...
    if rerank:
        ...
        dist = cuda_dist(feat, feat, metric)
        if hasattr(dist, 'asnumpy'):
            dist = dist.asnumpy()
...
def evaluate_CCPG(data, dataset, metric='euc'):
    ...
    for (p, probe_seq) in enumerate(probe_seq_dict[dataset]):
        ...
        distmat = cuda_dist(probe_x, gallery_x, metric)
        if hasattr(distmat, 'asnumpy'):
