...
class LossAggregator(nn.Cell):
    ...
    def __init__(self, loss_cfg) -> None:
        ...
        but modules it contains are properly registered, and will be visible by all Cell methods.
        All parameters registered in losses can be accessed by the method 'self.get_parameters()',
        ...
        """
        super().__init__()
        # In MindSpore, we use a dict to store losses, but they need to be registered as cells
        if is_dict(loss_cfg):
            loss_dict = {loss_cfg['log_prefix']: self._build_loss_(loss_cfg)}
        else:
            loss_dict = {cfg['log_prefix']: self._build_loss_(cfg) for cfg in loss_cfg}
        
        # Register losses as cells
        self.losses = nn.CellDict(loss_dict)
    ...
    def construct(self, training_feats):
        ...
        built losses and the value is mindspore.Tensor, then it is the computed loss to be added loss_sum.

        Args:
            training_feats: A dict of features. The same as the output["training_feat"] of the model.
        """
        loss_sum = ms.Tensor(0.0, dtype=ms.float32)
        loss_info = Odict()
        ...
        for k, v in training_feats.items():
            if k in self.losses:
                ...
                if isinstance(loss, ms.Tensor):
                    ...
                    loss = ms.Tensor(loss, dtype=ms.float32) * loss_func.loss_term_weight
                ...
                if isinstance(v, dict):
                    ...
                    _ = v.mean()
                    loss_info['scalar/%s' % k] = _.asnumpy()
                    loss_sum = loss_sum + _
